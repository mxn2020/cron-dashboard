...
import { Clock, CalendarClock, AlertCircle, Settings, LayoutDashboard } from "lucide-react";
import ThemeToggle from "./ThemeToggle";
export default function Sidebar() {
    const loc = useLocation();
    const links = [
        { to: "/", icon: LayoutDashboard, label: "Jobs Overview" },
        { to: "/timeline", icon: CalendarClock, label: "Execution Timeline" },
        { to: "/failures", icon: AlertCircle, label: "Failure Alerts" },
    ...
    return (<aside style={{ width: "var(--sidebar-width)", background: "var(--color-bg-card)", borderRight: "1px solid var(--color-border)", height: "100vh", position: "fixed", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "var(--space-6)", display: "flex", alignItems: "center", gap: "var(--space-3)", borderBottom: "1px solid var(--color-border)" }}>
            <div style={{ background: "var(--color-accent-primary)", borderRadius: "8px", padding: 6 }}><Clock size={20} style={{ color: "var(--color-text-inverse)" }} /></div>
            <strong style={{ fontSize: "1.1rem", fontWeight: 800, letterSpacing: "-0.5px" }}>Cron Dashboard</strong>
        </div>
